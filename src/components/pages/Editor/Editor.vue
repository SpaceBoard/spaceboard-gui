...
import FileBox from '@/components/parts/Editor/FileBox/FileBox.vue'
  ...
  methods: {
    ...
    setupPulse () {
      ...
      Pulse.setupRealtimeUpdaters({
        viewAttention (data) {
          console.log(data)
          if (data.spaceID === self.root[self.using].id) {
            self.setAttention(data.attention.x, data.attention.y, data.attention.scaler)
            self.viewAttention({ instant: false })
          }
        },
    ...
    createNewOnDrop ({ type, rect }) {
      ...
      switch (type) {
        ...
        case 'commentBox':
          ...
          break
        case 'file':
          newItem = Data.fileBox({
            pos: newPos
          })
          this.root.realtime.items['files'].push(
            newItem
          )
          break
      }
    ...
    initRealtime () {
      ...
      Pulse.onItemHydration({ spaceID: this.root.realtime.id })
        .then((data) => {
          this.root = data
          this.setupPulse()
          this.restoreCam()
    ...
    markCam () {
      ...
      Pulse.onAttention({ spaceID: this.root[this.using].id, attention: this.root[this.using].attention })
